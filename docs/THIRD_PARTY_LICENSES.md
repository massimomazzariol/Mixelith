...
| Mixelith abstract style references | Project-generated by `scripts/generate_style_reference_assets.dart` | Mixelith project asset | Yes | Yes | None beyond project license policy | Yes | Small abstract PNGs generated locally by code. No third-party artwork, no public artist labels, and no downloaded source image. |
| `tflite_flutter` package | https://pub.dev/packages/tflite_flutter | Apache-2.0 | Yes for package use through pub.dev dependency | Yes | Apache-2.0 license applies to package | No binary model file committed | Added for isolated local spike only. It does not include the selected model binaries. |

## Phase 1J-A Decision

The license gate did **not** pass. The official TensorFlow Lite style transfer model pair remains the best technical candidate, but Mixelith must not commit the model files until the exact binary license and redistribution terms are confirmed from an authoritative source.

## Phase 1J-B Local-Only Decision

Local developer evaluation is allowed without public model redistribution:

- The public repository may include the download script.
- The downloaded `.tflite` model files must stay ignored by Git.

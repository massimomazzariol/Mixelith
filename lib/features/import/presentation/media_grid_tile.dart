...
class MediaGridTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Semantics(
      button: true,
      label:
          'Photo ${asset.width} by ${asset.height}${isSelected ? ', selected' : ''}',
      child: InkWell(
        onTap: onTap,
        child: Stack(
          fit: StackFit.expand,
          children: [
            ...
            if (isSelected) _SelectedOverlay(selectionIndex: selectionIndex),
          ],
        ),
      ),
    );
...
class _SelectedOverlay extends StatelessWidget {
  const _SelectedOverlay({this.selectionIndex});
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      decoration: BoxDecoration(
        border: Border.all(color: MixelithColors.yellow, width: 3),
